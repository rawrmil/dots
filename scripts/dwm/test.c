...
// gcc -pthread -o status status.c -lX11
...
void* update_weather(void* arg) {
  while (1) {
    FILE* fp = popen("curl -s \"https://wttr.in/?format=%t,%p,%w\"", "r");
    if (fp != NULL) {
...
int main() {
  ...
  while (1) {
    snprintf(status_string, sizeof(status_string), "%s | %s | HELP ME | %s", weather, battery, datetime);
